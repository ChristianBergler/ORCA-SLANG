...
class Trainer:
    ...
    def __init__(
        ...
        self.prefix = prefix
        self.sample_annotated = sample_annotated
    ...
    def fit(
        ...
        device,
        val_metric: Union[int, str] = "loss",  # select model based on loss for default
    ...
    def train_epoch(self, epoch, train_loader, loss_fn, optimizer, device):
        ...
        for i, (features, features_denoised, label, _) in enumerate(train_loader):
            ...
            elif self.mode in ["autoencoder_denoised"]:
                network_input = features_denoised
                ground_truth = features_denoised
            elif self.mode in ["denoiser"]:
                network_input = features
                ground_truth = features_denoised
            ...
            network_output, latent = output

            loss = loss_fn(network_output, ground_truth)
            ...
            if i > (len(train_loader) - 2):
                self.writer.add_image(
                    tag="train" + "/NET_IN_"+label["file_name"][0],
                    img_tensor=prepare_img(
                        network_input[0], num_images=self.n_summaries, file_names=label["file_name"]
                    ),
                    global_step=epoch,
                )
                self.writer.add_image(
                    tag="train" + "/NET_OUT_"+label["file_name"][0],
                    img_tensor=prepare_img(
                        network_output[0], num_images=self.n_summaries, file_names=label["file_name"]
                    ),
                    global_step=epoch,
                )
    ...
    def test_epoch(self, epoch, test_loader, loss_fn, device, phase="val"):
        ...
        with torch.no_grad():
            ...
            for i, (features, features_denoised, label, _) in enumerate(test_loader):
                ...
                if i > (len(test_loader) - 2):
                    self.writer.add_image(
                        tag=phase + "/NET_IN_"+label["file_name"][0],
                        img_tensor=prepare_img(
                            network_input[0], num_images=self.n_summaries, file_names=label["file_name"]
                        ),
                    ...
                    self.writer.add_image(
                        tag=phase + "/NET_OUT_"+label["file_name"][0],
                        img_tensor=prepare_img(
                            network_output[0], num_images=self.n_summaries, file_names=label["file_name"]
                        ),
                        global_step=epoch,
